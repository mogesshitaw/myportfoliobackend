...
export const createProject = async (req, res) => {
    try {
        const { title, description, detailedDescription, technologies, domainName, liveUrl, githubUrl, status, imageUrl // አዲስ የምስል ዩአርኤል
         } = req.body;
        if (!req.user) {
        ...
        try {
            await emailService.sendProjectCreated(userId, project.title, project.id);
...
export const updateProject = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, detailedDescription, technologies, domainName, liveUrl, githubUrl, status, progress, imageUrl // አዲስ የምስል ዩአርኤል
         } = req.body;
        const userId = req.user.id;
        ...
        if (existingLike) {
            ...
            if (project.userId !== userId) {
                try {
                    ...
                    await emailService.sendNewLike(project.userId, user?.fullName || 'Someone', project.title, id);
    ...
    try {
        ...
        if (project.userId !== userId) {
            try {
                ...
                await emailService.sendNewComment(project.userId, req.user?.fullName || 'Someone', project.title, id, content);
